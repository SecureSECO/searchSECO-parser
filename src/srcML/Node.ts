...
export class TagData {
	...
	constructor(tag: string, textBefore: string, textInTag: string) {
		(this.tag = tag), (this.textBefore = textBefore), (this.textInTag = textInTag);
	}
}
...
export class Node {
	...
	public IsEqual(node: Node): boolean {
		...
		if (other.length != this._children.length) return false;
		for (let i = 0; i < this._children.length; i++) {
			if (!this._children[i].IsEqual(other[i])) return false;
		}
